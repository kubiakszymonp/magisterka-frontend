...
def create_mtld_charts():
    ...
    bars1 = ax1.bar(x - width/2, means_tokens, width, yerr=stds_tokens,
                   label='MTLD (tokeny)', color='#9b59b6', edgecolor='black',
                   capsize=4, error_kw={'linewidth': 1.5})
    bars2 = ax1.bar(x + width/2, means_lemmas, width, yerr=stds_lemmas,
                   label='MTLD (lematy)', color='#1abc9c', edgecolor='black',
                   capsize=4, error_kw={'linewidth': 1.5})
    ...
    ax1.set_ylabel('MTLD', fontsize=12, fontweight='bold')
    ax1.set_title('Różnorodność leksykalna MTLD\n(wyższy = bardziej zróżnicowane słownictwo, n=16 artykułów)', 
    ...
    plt.tight_layout()
    save_chart(fig1, "mtld_grouped")
    
    # ===== WYKRES 2: Boxplot z punktami =====
    ...
    bp1 = ax2.boxplot(tokens_data, labels=labels, patch_artist=True,
    ...
    bp2 = ax3.boxplot(lemmas_data, labels=labels, patch_artist=True,
                      medianprops={'color': 'black', 'linewidth': 2},
                      widths=0.6)
    ...
    ax3.set_ylabel('MTLD (lematy)', fontsize=12, fontweight='bold')
    ax3.set_title('Rozkład MTLD dla lemmatów (boxplot)', fontsize=14, fontweight='bold', pad=15)
    
    plt.tight_layout()
    save_chart(fig2, "mtld_boxplot")
    
    # ===== WYKRES 3: MTLD vs długość tekstu (z word_count) =====
    ...
    for version in versions:
        ...
        ax4.scatter(word_counts, mtld_values, c=VERSION_COLORS[version],
                   label=VERSION_LABELS[version], s=80, alpha=0.7,
                   edgecolors='black', linewidth=1)
    
    ax4.set_xlabel('Liczba słów w tekście', fontsize=12, fontweight='bold')
    ...
    sns.violinplot(data=df, x='Wersja', y='MTLD', hue='Typ', split=True, ax=ax5,
